...
| Rule synthesis from reclassifications | 🟡 partially shipped 2026-05-22 (Phase 8) | **Path B (content-edit synthesis) shipped:** `maury mine --synthesize` (implies `--crossref`) makes one extra LLM call for `rephrase-existing` / `investigate-why-not-followed` findings to propose a clearer/stronger **CLAUDE.md** wording (`src/maury/mining/synthesize.py`), included as an advisory "proposed rewrite" section in the run-branch finding block; operator applies by hand. **Path A (classify-rule synthesis into `rules.yaml`)** — the literal [ADR-0004](adr/0004-rule-engine-classification.md) Phase 8 — now **designed** in [ADR-0053](adr/0053-rule-driven-auto-placement.md) (rule-driven auto-placement, Proposed): `maury review` classifies each finding and `accept` auto-places it into the classified source file; reclassification re-files + synthesizes a `classify` rule. ADR-0053 also supersedes [ADR-0022](adr/0022-branch-per-mining-run.md)'s single-staging-file model for matched findings (manual queue persists for `profile=None`). Build tracked by ADR-0053's slices. Per the [ADR-0020](adr/0020-two-mining-modes-bulk-and-incremental.md) + ADR-0004 + ADR-0053 amendments. |
