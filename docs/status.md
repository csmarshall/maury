...
| `maury manifest resolve` | 🟡 partially shipped 2026-05-19 | Three-way structured merge engine + interactive resolver. Auto-resolves the additive case (both sides added different keys → keep all), surfaces every other ambiguity with the 4-option prompter (`a`=take A, `b`=take B, `s`=skip+keep ancestor, `q`=abort). Profile + host conflicts carry a **semantic summary** (e.g., "this profile is bound by N host(s) on A and M on B") so the user sees consequences before choosing. Validates the resolved manifest against the schema before atomic write. Gated by `~/.config/maury/.lock` advisory fcntl. `--non-interactive` refuses to prompt. Deferred (followups): author/timestamp/branch metadata per side, auto-merge integration into `maury sync`, edit-by-hand option, per-backend URL validator. Per [ADR-0024](adr/0024-manifest-concurrency-inclusive-merge.md). |
