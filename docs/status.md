...
| `maury manifest resolve` | 🟡 partially shipped 2026-05-19 | Three-way structured merge engine + interactive resolver. Auto-resolves the additive case (both sides added different keys → keep all), surfaces every other ambiguity with the 4-option prompter (`a`=take A, `b`=take B, `s`=skip+keep ancestor, `q`=abort). Validates the resolved manifest against the schema before atomic write. `--non-interactive` refuses to prompt. Deferred (followups): semantic-context-per-path ("N hosts will lose binding"), author/timestamp/branch metadata per side, auto-merge integration into `maury sync`, edit-by-hand option, per-backend URL validator, `~/.config/maury/.lock`. Per [ADR-0024](adr/0024-manifest-concurrency-inclusive-merge.md). |
...
| `maury repo init` | 🟡 partially shipped 2026-05-18 | Writes `.meta/maury-governance.json` (owners, pr_target, optional pr_standards/min_reviewers) + `.meta/maury-marker.json` (`layer: rules`, `agency_id`), creates initial semver tag (default `v1.0.0`), commits. Refuses if either .meta file exists; `--force` overwrites. **Post-commit auto-tagging hook (ADR-0038 step 6) deferred.** Owner-only access validation (ADR-0038 §"Why owner-only") deferred. Per [ADR-0038](adr/0038-precept-acquisition-model.md). |
