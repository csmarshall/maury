...
| `maury mine` | ✅ shipped 2026-05-21 | Transcript walker, LLM extractor, four-state crossref classification, AND **run-branch generation** per [ADR-0022](adr/0022-branch-per-mining-run.md). Pass `--write-run-branch --repo <base-repo>` after extraction to emit a `maury/run/<run-id>` branch with one commit per finding — each commit's diff is an append to `mining-findings.md` at the repo root, the body is the structured trailer block (`Kind:` / `Scope-Hint:` / `Confidence:` / `Crossref-State:` / `Content-Hash:` / `Source-Transcript:` / `Source-Window:` / `Mining-Run:`), and the subject is `maury: <kind> — <summary>`. Dedup via `git log --grep="^Content-Hash:"` (accepted history) + `git log --grep="^Rejected-Content-Hash:"` (rejection no-op commits per ADR-0022) — sub-second on multi-year history. Skips findings whose normalized (kind, scope, text) hash already lives anywhere reachable; doesn't create an empty branch if every finding was dedup-suppressed. Emits `mining_run_created` audit event. `maury review <run-id>` walks the branch (Phase 7, ✅ shipped 2026-05-21). |
| `maury review <run-id>` | ✅ shipped 2026-05-21 (auto-placement 2026-05-22) | Walks the `maury/run/<run-id>` branch oldest-first and builds `maury/review/<run-id>`. Per-finding interactive prompt (`a`ccept / re`c`lassify / `r`eject / `e`dit / `s`kip / `q`uit), plus `--accept-all` / `--reject-all [--reason]` batch flags. **Rule-driven auto-placement ([ADR-0053](adr/0053-rule-driven-auto-placement.md)):** when the repo carries `.meta/rules.yaml` + manifest, each finding is classified and `accept` auto-places it into the classified source file (manual queue for no-match); `c`lassify re-files + synthesizes a routing rule. Accepted/edited findings land as per-finding commits with their `Content-Hash` trailers preserved (applied by reconstructing the appended block — gap-safe vs. literal cherry-pick under the single-staging-file model, per the [ADR-0022](adr/0022-branch-per-mining-run.md) amendment); rejections land as one trailing no-op metadata commit (`Rejected-Content-Hash` + `Rejected-Reason`) written only on full-walk completion. Quitting keeps accepted work and resumes on re-run (skips already-applied findings). Refuses on stale base, pointing at `maury rebase-run`. Emits `review_completed` audit. Single-trust-boundary rw self-curation; cross-boundary promotion is `maury promote` ([ADR-0045](adr/0045-cross-trust-boundary-promotion.md)). |
| `maury rebase-run <run-id>` | ✅ shipped 2026-05-21 | Rebases a `maury/run/<run-id>` branch onto current `main` when `main` moved between mining and review. No-op when unmoved; aborts + errors on conflict (no mid-rebase stranding). Per [ADR-0022](adr/0022-branch-per-mining-run.md) §"Stale base handling". |
| `maury promote --from --to --to-mode` | ✅ shipped 2026-05-22 | Branch-fetch cross-trust-boundary promotion ([ADR-0045](adr/0045-cross-trust-boundary-promotion.md) §4). Curator with read on `--from` + write on `--to` reads the source's `maury/run/*` findings read-only, graph-checks each (`promotion.graph`: source-mode or ancestor only; siblings/unrelated refused before the curator sees them, with a "shared ancestor is `<LCA>`" hint), and runs the same `a/r/e/s/q` loop as `maury review` (`--accept-all` / `--reject-all [--reason]` too). Accepts land on `maury/promoted/<id>` in the dest with a `Promoted-From: <src>@<sha>` trailer + preserved `Content-Hash`/`Source-Mode` (reconstruct-and-append; the source SHA isn't reachable in the dest). Resume + mode-scoped rejection no-op commit. Emits `promotion_started`/`promotion_completed` audit. |
| `maury promote-review --from --to` | ✅ shipped 2026-05-22 | Proposal-queue counterpart ([ADR-0045](adr/0045-cross-trust-boundary-promotion.md) §1+§4). Walks the source repo's `proposals/promote-to-*/` queue (each proposal names its own `Promote-To` target), graph-checks defensively, same review loop. Proposals are written + committed at mine time (`maury mine --write-run-branch`) for findings whose destination is outside the host's writable subtree (graph-only `needs_promotion_proposal` test). |
| `maury focus use <path>` | ✅ shipped 2026-05-20 | Lightweight switch to a child mode within the host's trust boundary (e.g., `personal:consulting:acme`). Refuses if the target would cross a trust boundary (points at `mode deregister` + `mode bootstrap` per ADR-0039) or if active Claude Code sessions are running (override: `--force-active-session`). Updates the `active_focus` field on `host-identity.json`. Emits `focus_switched` on success or `focus_switch_refused` (with `precondition` payload naming the failed check). With no `<path>` argument, clears any active focus and resumes the registered mode. Sister verbs `maury focus current` and `maury focus list` ship same time. Per [ADR-0052](adr/0052-focus-the-lightweight-intra-trust-boundary-mode-switch.md). |
| `maury statusline` | ✅ shipped 2026-05-20 | One-line summary subcommand for Claude Code's `statusLine` setting. Reads `host-identity.json`; outputs `mode:<name>` (no focus set), `mode:<name> focus:<path>` (focus set), or `(no maury identity)` (no baseline yet). **Never crashes, never returns non-zero** — catches `BaseException` and falls back to `(maury statusline error)`. Wire it up manually by adding `"statusLine": {"type": "command", "command": "maury statusline"}` to your base-repo `settings.json` fragment (composed into `~/.claude/settings.json` at render time). The originally-proposed `maury init` auto-install was retracted 2026-05-20 (see ADR-0052 amendment-history). Per [ADR-0052](adr/0052-focus-the-lightweight-intra-trust-boundary-mode-switch.md). |
...
| `maury uninstall` | ✅ shipped 2026-05-18 | Strips `# maury-managed` hook entries from `settings.json`, deletes `~/.claude/bin/maury-*`, `~/.local/state/maury/`, and `~/.config/maury/host-id`. User content (CLAUDE.md, hand-authored hooks, repo clones) left intact. Confirmation prompt unless `--yes`. Per [ADR-0023 §8](adr/0023-hook-installation-and-tool-resolution.md). |
...
| Active-session detection + focus switch | ✅ shipped 2026-05-20 | `~/.local/state/maury/active-sessions.jsonl` event log + SessionStart/SessionEnd hooks ship per [ADR-0025](adr/0025-profile-switching-session-safeguards.md)'s carried-forward design; consumed by `maury focus use` per [ADR-0052](adr/0052-focus-the-lightweight-intra-trust-boundary-mode-switch.md) as the active-session precondition (override `--force-active-session`). The `maury mode use` design from ADR-0025 was superseded by [ADR-0039](adr/0039-bootstrap-and-host-lifecycle.md) — mode change is deregister + re-bootstrap, never a single command. |
...
| Proposal queue + review UI | ✅ shipped 2026-05-21 (Phase 7) | `maury review` + `maury rebase-run` per [ADR-0022](adr/0022-branch-per-mining-run.md). The "queue" is `git log main..maury/run/<run-id>`; no separate data structure. |
| Rule synthesis from reclassifications | ✅ shipped 2026-05-22 (Phase 8) | **Path A (classify-rule synthesis) shipped** per [ADR-0053](adr/0053-rule-driven-auto-placement.md): `maury review` classifies each finding, `accept` auto-places it into the classified source file (base `CLAUDE.md` / `profiles/<mode>/CLAUDE.md.fragment` / host fragment) or the manual queue (`profile=None`), and **reclassify** re-files + synthesizes a `classify` rule into `.meta/rules.yaml` (`src/maury/{placement,rules/synthesize}.py`). **Path B (content-edit synthesis) shipped:** `maury mine --synthesize` (implies `--crossref`) makes one extra LLM call for `rephrase-existing` / `investigate-why-not-followed` findings to propose a clearer/stronger **CLAUDE.md** wording (`src/maury/mining/synthesize.py`), included as an advisory "proposed rewrite" section in the run-branch finding block; operator applies by hand. Path A targets `rules.yaml` (the [ADR-0004](adr/0004-rule-engine-classification.md) classification ruleset / routing), Path B targets `CLAUDE.md` (the instructions) — distinct artifacts. ADR-0053 also supersedes [ADR-0022](adr/0022-branch-per-mining-run.md)'s single-staging-file model for matched findings (manual queue persists for `profile=None`). Per the [ADR-0020](adr/0020-two-mining-modes-bulk-and-incremental.md) + ADR-0004 + ADR-0053 amendments. |
| Cross-boundary promotion | ✅ shipped 2026-05-22 (Phase 9) | Both mechanisms per [ADR-0045](adr/0045-cross-trust-boundary-promotion.md): branch-fetch `maury promote` + proposal-queue `maury promote-review`, on the shared `promotion.graph` constraint. `push_policy: disabled` (§6) enforced; PR output (§7) ships as a lightweight `--open-pr` flag (push + `gh pr create` + `pr_opened`). **Still deferred to v1.0+:** full `pr` `RepoMode` tier (ADR-0033 — manifest model, validation, sync, and access checks), and proposals are committed on the run branch rather than a dedicated ref. |
| Audit log + push-policy enforcement | 🟡 partially shipped 2026-05-19 | Storage primitive `src/maury/audit_log.py` + `maury audit show` reader shipped. Event-kind wiring shipped 2026-05-19 for all V1 command sites: `sync_started` / `sync_completed` / `sync_aborted` / `drift_detected` / `render_applied` (sync), `init_completed` (init), `manifest_mutated` (mode bootstrap + deregister), `manifest_merge_resolved` (manifest resolve), `reconcile_action` (reconcile), `sessions_pruned` (sessions prune), `uninstall_completed` (uninstall). Added 2026-05-20: `focus_switched` / `focus_switch_refused` (focus use per ADR-0052). Added 2026-05-21: `mining_run_created` (mine --write-run-branch per ADR-0022) and `review_completed` (maury review per ADR-0022). Added 2026-05-22: `promotion_started` / `promotion_completed` (maury promote + promote-review per ADR-0045) and `pr_opened` (promote `--open-pr` per ADR-0045 §7 / ADR-0033). Best-effort: every call site wraps `log()` in `contextlib.suppress(AuditLogError)` so an audit-write failure can't abort the underlying operation. Remaining event kinds (`manifest_upgraded`, `claude_revert`, `subscription_added`, `subscription_pinned`, `backup_created`, `restore_completed`, `tool_use_logged`, `error`) wire up as their owning command sites ship. **Push-policy enforcement** is a separate followup. |
...
`~/.local/state/maury/` write contract are all verified safe.
