...
| `maury manifest resolve` | 🟡 partially shipped 2026-05-19 | Three-way structured merge engine + interactive resolver. Auto-resolves the additive case (both sides added different keys → keep all), surfaces every other ambiguity with the 4-option prompter (`a`=take A, `b`=take B, `s`=skip+keep ancestor, `q`=abort). Profile + host conflicts carry a **semantic summary** (e.g., "this profile is bound by N host(s) on A and M on B") so the user sees consequences before choosing. **Author/timestamp/branch metadata** for both sides is fetched from HEAD/MERGE_HEAD and surfaced at the top of the session. Validates the resolved manifest against the schema before atomic write. Gated by `~/.config/maury/.lock` advisory fcntl. `--non-interactive` refuses to prompt. Deferred (followups): auto-merge integration into `maury sync`, edit-by-hand option, per-backend URL validator. Per [ADR-0024](adr/0024-manifest-concurrency-inclusive-merge.md). |
...
| `maury agency validate` | ✅ shipped 2026-05-18 (basic; marker-graph checks planned) | Canonical name; delegates to the same manifest-validation handler today. Marker-file integrity, sublayer-graph cycles, and host-mode mismatches across an agency are planned per [ADR-0040](adr/0040-render-pipeline.md). |
...
| `maury repo init` | ✅ shipped 2026-05-19 | Writes `.meta/maury-governance.json` (owners, pr_target, optional pr_standards/min_reviewers) + `.meta/maury-marker.json` (`layer: rules`, `agency_id`), creates initial semver tag (default `v1.0.0`), installs the auto-patch-bump post-commit hook (ADR-0038 step 6), commits. `--no-hook` skips the hook; `--force` overwrites .meta files + a non-maury post-commit hook. Owner-only access validation (ADR-0038 §"Why owner-only") still deferred — in practice users run this against their own repos. Per [ADR-0038](adr/0038-precept-acquisition-model.md). |
