...
| Audit log + push-policy enforcement | 🟡 partially shipped 2026-05-19 | Storage primitive `src/maury/audit_log.py` + `maury audit show` reader shipped. Event-kind wiring shipped 2026-05-19 for all V1 command sites: `sync_started` / `sync_completed` / `sync_aborted` / `drift_detected` / `render_applied` (sync), `init_completed` (init), `manifest_mutated` (mode bootstrap + deregister), `manifest_merge_resolved` (manifest resolve), `reconcile_action` (reconcile), `sessions_pruned` (sessions prune), `uninstall_completed` (uninstall). Added 2026-05-20: `focus_switched` / `focus_switch_refused` (focus use per ADR-0052). Best-effort: every call site wraps `log()` in `contextlib.suppress(AuditLogError)` so an audit-write failure can't abort the underlying operation. Remaining event kinds (`manifest_upgraded`, `claude_revert`, `mining_run_created`, `review_completed`, `promotion_started`, `promotion_completed`, `pr_opened`, `subscription_added`, `subscription_pinned`, `backup_created`, `restore_completed`, `tool_use_logged`, `error`) wire up as their owning command sites ship. **Push-policy enforcement** is a separate followup. |
