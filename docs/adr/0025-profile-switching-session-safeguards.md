# ADR-0025: Profile switching with session-boundary safeguards
...
*active* profile at a time, with the option to switch via
`maury profile use <name>`. That CLI surface was named but never
specified. This ADR fills the gap.

The risk profile-switching introduces is real and specific:

> If a host can switch from `personal` to `work` (or any
> cross-boundary pair) while a Claude Code session is running, the
> running session has the **old** profile's CLAUDE.md and skills
> loaded into its working memory. The user, having run
> `maury profile use work`, may believe the assistant is operating
...
`maury profile use <name>` refuses to act unless three preconditions
hold. Each precondition has a deliberate "loud override" affordance
for advanced users.

### Precondition 0 — Host's profile is not locked

If the manifest entry for this host has `lock: true` (per
[ADR-0001](0001-n-profiles.md)'s opt-out), `maury profile use`
...
implication that profile changes on this host are now possible).
...
file content on disk differs from `last-render.json`. If a profile
switch were allowed with unresolved drift, the user's hand-edits
made under profile A would either:

- Be overwritten by profile B's render (silent data loss → tenet 1 violation), or
- Be carried into profile B's namespace (cross-profile leakage of
  edits the user explicitly made in A's context).

Neither is acceptable. Profile switch refuses with a pointer to
`maury reconcile`. The user resolves drift first (adopt as
proposal on the OLD profile's run branch, mark hand-managed under
the OLD profile, or revert), THEN switches.
...
```
maury profile use work --force --i-understand-cross-boundary-risk
```

Both flags are required. `--force` alone exits 1 with:

```
ERROR: --force is not sufficient for profile switch.
...
treat `--force` as sufficient on its own. Profile switch warrants
...
1. **Writes an audit log entry** (Phase 10):
   `{ts, host_id, from_profile, to_profile, sessions_state, force_flag_used}`.
2. **Renders the new CLAUDE.md with a profile banner.** Two layers,
   ...
   - A structured HTML-comment marker at the top:
     `<!-- maury-managed: active profile work (profile_<short>...) -->`
   - A human-visible sentence near the top of CLAUDE.md:
     *"You are operating in the **work** profile context."*
   This lets Claude's first response in any new session confirm the
   active profile back to the user — a verification surface beyond
   what's written to disk.
3. **Surfaces `maury status`** (the v1 skill from ADR-0017) which
   returns the active profile + last-switch timestamp + any pending
   captures or drift. The user can invoke this mid-session to
   confirm boundary state.
4. **Updates `last-render.json`** with the new profile's content
...
- **A user who manually edits `~/.claude/CLAUDE.md` and `settings.json`**
  to a different profile's content, bypassing `maury profile use`,
...
- **A session resumed via `claude --resume <id>`** inherits the
  ...
  *of* a past session whose lifetime spanned a profile switch.
  Documented; the audit log of past profile switches lets a
  curious user reconstruct whether a given resumed session is at
  risk.
- **Past transcripts under `~/.claude/projects/<hash>/`** persist
  on disk after a profile switch. They are visible to mining
  ([ADR-0005](0005-local-only-mining.md)). Profile-aware mining is
  a separate concern (gap E).

## Consequences

- **The cross-boundary leakage path is closed by default.** A
  no-flag `maury profile use` cannot cause a running session to
  see content from the new profile or vice versa, because it
...
- **Banner-mismatch detection skill.** A `maury check-profile`
