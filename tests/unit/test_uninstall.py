...
def test_run_uninstall_removes_state_dir(tmp_path: Path) -> None:
    target = tmp_path / "claude"
    home = tmp_path / "home"
    _seed_full_install(target, home)
    summary = run_uninstall(target_dir=target, home_dir=home)
...
def test_run_uninstall_idempotent_when_already_clean(tmp_path: Path) -> None:
    """Running uninstall twice should not error and second run is a no-op."""
    target = tmp_path / "claude"
    home = tmp_path / "home"
    _seed_full_install(target, home)
    run_uninstall(target_dir=target, home_dir=home)
    ...
    assert not summary.state_dir_removed
    assert not summary.host_id_removed


def test_run_uninstall_handles_completely_clean_host(tmp_path: Path) -> None:
    """No settings.json, no bin dir, no state, no host-id → all no-ops, no errors."""
    target = tmp_path / "claude"
    home = tmp_path / "home"
    target.mkdir()
    home.mkdir()
    summary = run_uninstall(target_dir=target, home_dir=home)
    assert summary == UninstallSummary(
        settings_hooks_removed=0,
        settings_hooks_kept=0,
        bin_files_removed=(),
        state_dir_removed=False,
        host_id_removed=False,
