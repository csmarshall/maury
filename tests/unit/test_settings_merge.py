...
def test_merge_dicts_child_overrides_keys():
...
def test_merge_dicts_child_adds_keys():
    parent = {"theme": "dark"}
    child = {"newSetting": 42}
    out = deep_merge(parent, child)
...
def test_merge_dicts_recurses_into_nested():
...
def test_merge_arrays_appends_unique():
    parent = ["a", "b"]
    child = ["c"]
    out = deep_merge(parent, child)
    assert out == ["a", "b", "c"]
...
def test_merge_arrays_dedupes_scalars():
    parent = ["a", "b"]
    child = ["b", "c"]
    out = deep_merge(parent, child)
    assert out == ["a", "b", "c"]
...
def test_merge_arrays_dedupes_objects_by_deep_equality():
...
def test_merge_replace_marker_replaces_array():
    parent = ["a", "b", "c"]
    child = [{"$replace": ["x", "y"]}]
    out = deep_merge(parent, child)
    assert out == ["x", "y"]
...
def test_merge_replace_marker_must_wrap_a_list():
    parent = ["a"]
...
def test_merge_pure_function_does_not_mutate():
    parent = {"a": [1, 2], "b": {"x": 1}}
    child = {"a": [3], "b": {"y": 2}}
...
def test_merge_layers_three_layers():
    layers = [
...
def test_merge_layers_empty_returns_empty_dict():
    assert merge_layers([]) == {}


def test_merge_layers_invalid_json_raises():
    layers = [
        ("base", '{"valid": true}'),
...
def test_merge_layers_non_object_top_level_raises():
    layers = [("base", "[1, 2, 3]")]
    with pytest.raises(SettingsMergeError) as ei:
        merge_layers(layers)
...
def test_collect_finds_settings_json(tmp_path):
    (tmp_path / "settings.json").write_text('{"theme": "dark"}')
    out = collect_settings_layers([("base", tmp_path)])
    assert out == [("base", '{"theme": "dark"}')]


def test_collect_finds_fragment_when_settings_json_absent(tmp_path):
...
def test_collect_prefers_settings_json_over_fragment(tmp_path):
...
def test_collect_skips_layers_with_no_settings(tmp_path):
...
def test_render_engine_emits_merged_settings(tmp_path):
...
def test_render_engine_skips_settings_when_no_layers_provide(tmp_path):
...
def test_render_engine_records_settings_error_on_bad_json(tmp_path):
